...
public class SupervisorResource {
    ...
    public SupervisorResource(SupervisorService supervisorService, SupervisorQueryService supervisorQueryService) {
    ...
    public ResponseEntity<List<Supervisor>> getAllSupervisors(SupervisorCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Supervisors by criteria: {}", criteria);
        Page<Supervisor> page = supervisorQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /supervisors/count} : count all the supervisors.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/supervisors/count")
    public ResponseEntity<Long> countSupervisors(SupervisorCriteria criteria) {
        log.debug("REST request to count Supervisors by criteria: {}", criteria);
        return ResponseEntity.ok().body(supervisorQueryService.countByCriteria(criteria));
    }
