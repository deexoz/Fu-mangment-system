...
/**
 * REST controller for managing {@link com.fu.pums.domain.File}.
 */
@RestController
@RequestMapping("/api")
public class FileResource {
     ...
     */
    @GetMapping("/files")
    public ResponseEntity<List<File>> getAllFiles(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("resultdata-is-null".equals(filter)) {
            log.debug("REST request to get all Files where resultData is null");
            return new ResponseEntity<>(fileService.findAllWhereResultDataIsNull(),
                    HttpStatus.OK);
        }
